...
function demo(; return_data = false)
    ...
    y2, _ = perform_separation(x,Shibbs(2, 1000, 1))
    y3, _ = perform_separation(x,Picard(2, 3, 200, 1e-6, 1e-2, 10, false))
    p1 = plot_dataset(y1; title="Jade - Estimated Source Signals")
    p2 = plot_dataset(y2; title="Shibbs - Estimated Source Signals")
    p3 = plot_dataset(y3; title="Picard - Estimated Source Signals")
